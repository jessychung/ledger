...
// ---- Home ----
function HomeScreen({ onAdd, onPickMonth, onOpenExpense }) {
  const store = useStore();
  const [activeMonth, setActiveMonth] = React.useState(nowMonthKey());
  const [chartMode, setChartMode] = React.useState("donut"); // donut | bars
  ...
  // last 6 months for the strip / sparkline
  ...
  const monthValues = months.map((m) => totals[m] || 0);
        ...
        <div className="between" style={{ alignItems: "center", flexWrap: "wrap", gap: 10 }}>
          <div className="row gap-3" style={{ alignItems: "center" }}>
            ...
            onClick={() => {
              ...
              if (i > 0) setActiveMonth(months[i - 1]);
            }}
            ...
            onClick={() => {
              const i = months.indexOf(activeMonth);
              if (i < months.length - 1) setActiveMonth(months[i + 1]);
            }}
      ...
      <div className="card" style={{ padding: 28 }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 28, alignItems: "center" }}>
          <div className="stack gap-3">
            ...
            <div className="row gap-3" style={{ flexWrap: "wrap", color: "var(--muted)", fontSize: 13 }}>
              ...
              <span style={{ color: "var(--alert)" }}>
                    {sym}{Math.abs(remaining).toFixed(0)} over · of {sym}{budget.toLocaleString()} budget
                  </span> :

              <span>
                    {sym}{remaining.toFixed(0)} of {sym}{budget.toLocaleString()} left
                  </span>)
          ...
          <div style={{ position: "relative", display: "grid", placeItems: "center" }} className="hero-ring">
            <ProgressRing pct={Math.min(pct, 1)} size={170} stroke={10}
            color={ringColor}
            track="var(--surface-2)" />
            <div style={{ position: "absolute", textAlign: "center" }}>
              <div className="serif" style={{ fontSize: 30, lineHeight: 1 }}>{Math.round(pct * 100)}%</div>
              <div className="muted" style={{ fontSize: 11, letterSpacing: "0.08em", textTransform: "uppercase", marginTop: 4 }}>
                used
              </div>
            </div>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "auto 1fr", gap: 28, alignItems: "center" }} className="brk-inner">
                ...
                <>
                        <div className="muted" style={{ fontSize: 11, letterSpacing: "0.06em", textTransform: "uppercase" }}>
                          {breakdownData[hoverIdx].label}
                        </div>
                        <div className="serif" style={{ fontSize: 22, marginTop: 2 }}>
                          {sym}{Math.round(breakdownData[hoverIdx].value)}
                        </div>
                      </> :
...
// ---- Activity (history list with month picker) ----
function ActivityScreen({ initialMonth, onOpenExpense }) {
  const store = useStore();
  ...
  // last 6 months rolling window for the trend chart
    ...
    <div className="stack gap-4">
      <div className="between">
        ...
        <div className="stack" style={{ alignItems: "flex-end" }}>
          ...
          <div className="serif" style={{ fontSize: "38px" }}>{sym}{Math.round(total).toLocaleString()}</div>
        </div>
      </div>
      ...
      <div className="card">
        <div className="between" style={{ marginBottom: 14 }}>
          <div>
            ...
            <div className="muted" style={{ fontSize: 12, marginTop: 2 }}>Tap a bar to switch month</div>
          </div>
          {budget > 0 &&
        ...
        <div style={{ position: "relative" }}>
          <MonthBars
            months={trendMonths} values={trendValues} budget={budget}
            currencySym={sym} activeKey={mk}
            onPick={setMk} />
          ...
          {budget > 0 && (() => {
            ...
            return (
              <div style={{
                ...
                height: 1, borderTop: "1px dashed var(--muted-2)",
                pointerEvents: "none"
              }} />);
      ...
      <div className="row gap-2" style={{ flexWrap: "wrap" }}>
        <div style={{ position: "relative", flex: "1 1 200px", minWidth: 180 }}>
          ...
          <input className="input" placeholder="Search notes..."
...
function ExpenseForm({ initial, onSave, onCancel, onDelete }) {
  const store = useStore();
  const [amount, setAmount] = React.useState(initial?.amount ? String(initial.amount) : "");
  const [category, setCategory] = React.useState(initial?.category || "groceries");
    ...
    <div className="stack gap-4">
        ...
        <div style={{ display: "flex", alignItems: "baseline", gap: 8, padding: "10px 14px",
          ...
          <input
            ...
            style={{ flex: 1, border: 0, outline: "none", background: "transparent",
              fontSize: 36, color: "var(--ink)", padding: 0,
      ...
      <div>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
          ...
          <CategoryChip key={c.id} cat={c} selected={category === c.id}
      ...
      <div className="row gap-2" style={{ marginTop: 6 }}>
        ...
        style={{ opacity: valid ? 1 : 0.4, flex: 1 }}
        onClick={submit}>
    ...
    <div className="stack gap-4">
      ...
      <div className="card">
        ...
        {store.state.fixed.map((f) => {
          ...
          return (
            ...
            style={{ background: "none", border: 0, borderBottom: "1px solid var(--line)", width: "100%", textAlign: "left", padding: "12px 0", cursor: "pointer", borderWidth: "0px", borderTopStyle: "none", borderRightStyle: "none", borderLeftStyle: "none", borderTopColor: "currentcolor", borderRightColor: "currentcolor", borderLeftColor: "currentcolor" }}>
              ...
              <div className="amt">{sym}{hasCents(store.state.settings.currency) ? f.amount.toFixed(2) : Math.round(f.amount).toLocaleString()}</div>
            </button>);
      ...
      title={editing === "new" ? "New fixed expense" : "Edit fixed expense"}>
        <FixedForm
          ...
          onSave={(data) => {
            if (editing === "new") store.addFixed(data);else
            store.updateFixed(editing, data);
            setEditing(null);
          }}
...
function FixedForm({ initial, onSave, onCancel, onDelete }) {
  const store = useStore();
    ...
    <div className="stack gap-4">
      ...
      <div>
        ...
        <div style={{ display: "flex", alignItems: "baseline", gap: 8, padding: "10px 14px",
          ...
          placeholder="0.00"
          style={{ flex: 1, border: 0, outline: "none", background: "transparent",
            fontSize: 22, color: "var(--ink)", padding: 0, fontFamily: '"Instrument Serif", serif' }} />
        </div>
      </div>
      <div>
        <div className="field-label">Category</div>
    ...
    <div className="stack gap-4">
      ...
      <div className="card stack gap-4">
        <div>
          ...
          <div style={{ display: "flex", alignItems: "baseline", gap: 8, padding: "10px 14px",
            border: "1px solid var(--line)", borderRadius: "var(--r-md)", background: "var(--bg)" }}>
            ...
            style={{ flex: 1, border: 0, outline: "none", background: "transparent",
              fontSize: 24, color: "var(--ink)", padding: 0, fontFamily: '"Instrument Serif", serif' }} />
          </div>
          <div className="muted" style={{ fontSize: 12, marginTop: 6 }}>
            Updates the progress ring on the homepage.
          </div>
        </div>
      ...
      <div className="card stack gap-3">
        <div className="between">
          ...
          <button className="btn" style={{ padding: "8px 12px", fontSize: 13 }}
      ...
      title={editingCat === "new" ? "New category" : "Edit category"}>
        <CategoryForm
          ...
          onSave={(data) => {
            if (editingCat === "new") store.addCategory(data);else
            store.updateCategory(editingCat, data);
            setEditingCat(null);
          }}
          onDelete={() => {
            ...
            const msg = usage > 0 ? `Delete this category? ${usage} item(s) using it will be reassigned.` : "Delete this category?";
      ...
      <div className="card stack gap-3">
        <div>
          ...
          <div className="muted" style={{ fontSize: 12 }}>Stored locally on this device.</div>
        </div>
        <div className="row gap-2" style={{ flexWrap: "wrap" }}>
...
function CategoryForm({ initial, canDelete, onSave, onDelete }) {
  ...
  const [customIcons, setCustomIcons] = React.useState(() => {
    const arr = [];
    if (initial?.icon && initial.icon.startsWith("svg:")) arr.push(initial.icon);
    return arr;
  });
  const [genState, setGenState] = React.useState("idle"); // idle | loading | error
  const [customColor, setCustomColor] = React.useState(() => {
    return initial?.color && !initial.color.startsWith("var(") ? initial.color : "#7a8770";
  });
...
Rules:
- 1 to 3 SVG path "d" strings, designed for a 24x24 viewBox
- Stroke-only (the SVG will be rendered with stroke=1.6, no fill, rounded joins/caps)
- Keep paths simple — under 12 commands each
- Centered, with ~2px padding from edges
- Recognizable silhouette of the concept`;
      const text = await window.claude.complete(prompt);
      const m = text.match(/\{[\s\S]*\}/);
      if (!m) throw new Error("no json");
      const obj = JSON.parse(m[0]);
      if (!obj.paths || !Array.isArray(obj.paths) || obj.paths.length === 0) throw new Error("no paths");
      const enc = "svg:" + obj.paths.join("|");
    ...
    <div className="stack gap-4">
      ...
      <div>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
          ...
          style={{ width: 32, height: 32, borderRadius: "50%", background: c,
            ...
            outline: "1px solid var(--line)", cursor: "pointer" }} aria-label={c} />
          )}
          <label style={{
            position: "relative",
      ...
      <div>
        <div className="between" style={{ marginBottom: 6 }}>
          ...
          <button type="button" onClick={generateIcon}
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          ...
          title={name.startsWith("svg:") ? "AI-generated" : name}
          style={{ width: 40, height: 40, borderRadius: 10,
            ...
            display: "grid", placeItems: "center", cursor: "pointer", position: "relative" }} aria-label={name}>
              <Icon name={name} size={16} color="currentColor" />
              {name.startsWith("svg:") && icon !== name &&
            <span style={{ position: "absolute", top: -3, right: -3, color: "var(--accent)", background: "var(--surface)", borderRadius: "50%", width: 12, height: 12, display: "grid", placeItems: "center" }}>
                  <Icon name="sparkle" size={8} color="currentColor" />
                </span>
            }
            </button>
          )}
        ...
        <div style={{ fontSize: 12, color: "var(--alert)", marginTop: 6 }}>
            Couldn't generate — try again or pick from above.
          </div>
        }
      </div>
      <div className="row gap-2">
...
Object.assign(window, {
  HomeScreen, ActivityScreen, FixedScreen, SettingsScreen,
  ExpenseForm
});
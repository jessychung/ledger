...
// --- Progress ring ---
function ProgressRing({ pct, size = 240, stroke = 10, color, track }) {
...
function Donut({ data, size = 200, stroke = 22, hoverIdx, onHover }) {
  ...
  return (
    <svg width={size} height={size} className="donut">
      ...
      {data.map((d, i) => {
        ...
        const len = Math.max(0, frac * c - 2); // small gap
...
// --- Sparkline ---
function Sparkline({ values, width = 220, height = 50, color = "var(--ink)", area = true }) {
...
function MonthBars({ months, values, budget, currencySym = "$", onPick, activeKey }) {
...
// --- Category chip (with icon + label, used in chooser) ---
...
// --- Expense row ---
function ExpenseRow({ expense, cat, currencySym, currency, onClick }) {
...
// --- Sheet (bottom sheet on mobile, modal on desktop) ---
...
Object.assign(window, {
  Icon, ProgressRing, Donut, Sparkline, MonthBars,
  CategoryChip, ExpenseRow, Sheet, useToast,
});
